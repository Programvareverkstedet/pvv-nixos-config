...
# NOTE: there are more keys to be added here, but we kinda only ever
#       expect the barcode scanner to send digits + enter. The stuff
#       below is already somewhat overkill.
UNSHIFTED: dict[int, str] = {}
...
for code, plain, shifted in DIGIT_ROW:
    UNSHIFTED[code] = plain
    SHIFTED[code] = shifted

for letter in string.ascii_uppercase:
    code = getattr(ec, f"KEY_{letter}")
    UNSHIFTED[code] = letter.lower()
    SHIFTED[code] = letter.upper()

SHIFT_KEYS = {ec.KEY_LEFTSHIFT, ec.KEY_RIGHTSHIFT}
...
def main() -> None:
    ...
    for event in device.read_loop():
        ...
        char = (SHIFTED if shift_held else UNSHIFTED).get(event.code)
        if char is not None:
            line.append(char)
